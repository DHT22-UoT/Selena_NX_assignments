...
final_solution <- df %>%
  filter(!is.na(country)) %>% #excluding rows with missing country info #can also use the same filter/grepl function here to remove NAs
  filter(!is.na(shape))  %>% #excluding rows with missing shape info #Code works as all NAs were removed
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H%M%S")) %>% #Convert datetime column into appropriate formats
 #Should remove the time as well so the formats of both dates match
...
avg_report_delay_table <- final_solution %>%
  group_by(country) %>% #I Also had the same table with 5 countries after all the filters
...
final_solution <- final_solution %>%
  filter(duration..seconds. <= 1432.5) %>% #You can also change the unit from seconds to min or hours so the range is more appropriate
